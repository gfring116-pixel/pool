...
def save_data(data):
    ...
    with open('military_data.json', 'w') as f:
...
def add_points(user_id, points, reason, awarded_by):
    """Add points to user"""
    data = load_data()
    user_id = str(user_id)
    ...
    save_data(data)
    return data["users"][user_id]["total_points"]
...
def get_next_rank(current_rank, total_points):
...
def ai_determine_points(description):
...
async def award_points(ctx, user: discord.Member, points: int = None, *, description: str):
    ...
    if points is None:
        points = ai_determine_points(description)
    else:
        # Ensure points are within valid range
        points = max(1, min(5, points))

    # Award points
    total_points = add_points(user.id, points, description, ctx.author.id)
    ...
    embed.add_field(name="This Month", value=f"{user_data['monthly_points'].get(get_current_month(), 0)} points", inline=True)
    embed.set_thumbnail(url=user.display_avatar.url)

    await ctx.respond(embed=embed)

@bot.slash_command(name="pass_exam", description="Mark a user as having passed the sergeant exam")
async def pass_exam(ctx, user: discord.Member):
    ...
    data = load_data()
    user_id = str(user.id)
    if user_id not in data["exams_passed"]:
        data["exams_passed"].append(user_id)
        save_data(data)
...
async def my_points(ctx):
    ...
    current_rank = get_user_rank(ctx.author)
    next_rank, points_needed, requires_exam = get_next_rank(current_rank, user_data['total_points'])
...
async def check_points(ctx, user: discord.Member):
...
async def leaderboard(ctx, period: str = "total"):
    ...
    for user_id, user_data in data["users"].items():
        try:
            ...
            if member:
                ...
                current_rank = get_user_rank(member)
...
async def promote(ctx, user: discord.Member):
    ...
    if not is_host(ctx.author) and user != ctx.author:
        embed = discord.Embed(
            title="❌ Access Denied",
    ...
    data = load_data()
    if requires_exam and str(user.id) not in data["exams_passed"]:
        embed = discord.Embed(
            title="❌ Exam Required",
...
# Owner Commands
@bot.slash_command(name="owner_add_points", description="[OWNER] Add points to any user")
async def owner_add_points(ctx, user: discord.Member, points: int, *, reason: str = "Manual addition by bot owner"):
    ...
    total_points = add_points(user.id, points, reason, ctx.author.id)
    user_data = get_user_data(user.id)
    current_month = get_current_month()
    ...
    embed.add_field(name="New Total", value=f"{total_points} points", inline=True)
    embed.add_field(name="Monthly Total", value=f"{user_data['monthly_points'].get(current_month, 0)} points", inline=True)

    await ctx.respond(embed=embed)

@bot.slash_command(name="owner_remove_points", description="[OWNER] Remove points from any user")
async def owner_remove_points(ctx, user: discord.Member, points: int, *, reason: str = "Manual removal by bot owner"):
